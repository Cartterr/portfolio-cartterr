...
if "%PM%"=="npm" (
  ...
  start "backend" cmd /c %ENV_VARS% npm --prefix backend run dev
) else (
  if "%PM%"=="yarn" (
    echo 🧪 backend exec: yarn --cwd backend dev
    start "backend" cmd /c %ENV_VARS% yarn --cwd backend dev
  ) else (
    echo 🧪 backend exec: pnpm --dir backend dev
    start "backend" cmd /c %ENV_VARS% pnpm --dir backend dev
  )
)

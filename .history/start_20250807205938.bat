...
if not exist "%ROOT%node_modules\.bin\cross-env.cmd" call :install_root

call :free_port %BACKEND_PORT%
call :free_port %FRONTEND_PORT%

set "BACK_CMD=cross-env NODE_ENV=development PORT=%BACKEND_PORT% FRONTEND_URL=http://localhost:%FRONTEND_PORT% yarn --cwd backend dev"
set "FRONT_CMD=cross-env NODE_ENV=development VITE_API_URL=%API_URL% yarn --cwd frontend dev --port %FRONTEND_PORT%"
echo 🧪 running both services with concurrently
"%ROOT%node_modules\.bin\concurrently.cmd" -n backend,frontend -c blue,magenta "%BACK_CMD%" "%FRONT_CMD%"

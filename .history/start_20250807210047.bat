...
set "BACK_CMD=cmd /c \"set NODE_ENV=development&& set PORT=%BACKEND_PORT%&& set FRONTEND_URL=http://localhost:%FRONTEND_PORT%&& yarn --cwd backend dev\""
set "FRONT_CMD=cmd /c \"set NODE_ENV=development&& set VITE_API_URL=%API_URL%&& yarn --cwd frontend dev --port %FRONTEND_PORT%\""

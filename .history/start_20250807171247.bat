...
start /b cmd /c "cd /d backend && set NODE_ENV=development && set PORT=%BACKEND_PORT% && set FRONTEND_URL=http://localhost:%FRONTEND_PORT% && if %PM%==npm (npm run dev) else if %PM%==yarn (yarn dev) else (pnpm dev)"

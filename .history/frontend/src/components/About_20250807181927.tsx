...
const About = () => {
  ...
  const nextImage = () => setActiveIdx(idx => (idx === null ? 0 : (idx + 1) % images.length))
  const prevImage = () => setActiveIdx(idx => (idx === null ? 0 : (idx - 1 + images.length) % images.length))
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <div className="relative w-full h-96 bg-primary-secondary rounded-3xl flex items-center justify-center overflow-hidden glass-effect">
                      ...
                      <div className="h-full w-full relative">
                        {images.map((img, idx) => (
                          <img
                            ...
                            src={img.url}
                            alt={img.name}
                            className="absolute top-0 left-0 h-full w-full object-cover transition-all duration-700"
                            style={{
                              opacity: idx === 0 ? 1 : 0,
                              transform: idx === 0 ? 'translateX(0)' : 'translateX(40px)'
                            }}
              ...
              {images.length > 1 && (
                <button onClick={() => setActiveIdx(0)} className="absolute inset-0" aria-label="Open gallery" />
              )}
            </div>
          </motion.div>
      ...
      {activeIdx !== null && images.length > 0 && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80" onClick={() => setActiveIdx(null)}>
          <div className="relative max-w-5xl w-[92vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="aspect-[16/9] w-full overflow-hidden rounded-xl bg-black/40">
              <img src={images[activeIdx].url} alt={images[activeIdx].name} className="w-full h-full object-contain" />
            </div>
            {images.length > 1 && (

...
const Experience = () => {
  ...
  const Gallery: React.FC<{ images: { name: string; url: string }[] }> = ({ images }) => {
    ...
    return (
      <div
        className="group relative w-full aspect-[5/4] bg-primary-secondary rounded-3xl flex items-center justify-center overflow-hidden glass-effect hover:scale-[1.01] transition-transform duration-300"
      ...
      >
        ...
        {images.length > 0 && (
          <button onClick={() => setActiveIdx(current)} className="absolute inset-0 cursor-zoom-in" aria-label="Open gallery" />
        )}
          ...
          <AnimatePresence>
            {activeIdx !== null && (
              ...
              >
                ...
                >
                  <button aria-label="Close" className="absolute -top-12 right-0 text-white/80 hover:text-white" onClick={() => setActiveIdx(null)}>
                    <X className="w-9 h-9" />
                  </button>
